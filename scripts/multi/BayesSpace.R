...
run_BayesSpace_multi <- function(samples, cnt_paths, xy_paths, genes_paths, cluster_num) {
    sces <- c()
    XY <- data.frame()
    xy_list <- list()
    genes_list <- list()
    ...
    sce.combined <- spatialCluster(sce.combined, use.dimred = "PCA", q = cluster_num, nrep = 10000)

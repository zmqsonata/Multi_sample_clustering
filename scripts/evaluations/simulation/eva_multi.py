...
def main(samples, cluster_num, method):
    ...
    for sample in samples:
        cnt_paths.append("../../../results/tmp/simulation1/" + sample + "_cnt.mtx")
        xy_paths.append("../../../results/tmp/simulation1/" + sample + "_xy.csv")
        genes_paths.append("../../../results/tmp/simulation1/" + sample + "_genes.csv")
    ...
    if type(clusters) == 'rpy2.robjects.vectors.ListVector':
        for sample in clusters.names:
            ...
            df.to_csv(os.path.join("../../../results/tmp/simulation1/multi", '_'.join([sample, method]) + '.csv'))
    else:

...
def main(sample, cluster_num, method):
    ...
    cnt_paths = ["../../../results/tmp/simulation1/" + sample + "_cnt.mtx"]
    xy_paths = ["../../../results/tmp/simulation1/" + sample + "_xy.csv"]
    genes_paths = ["../../../results/tmp/simulation1/" + sample + "_genes.csv"]
    ...
    if type(clusters) == 'rpy2.robjects.vectors.ListVector':
        for sample in clusters.names:
            ...
            df.to_csv(os.path.join("../../../results/tmp/simulation1/simple", '_'.join([sample, method]) + '.csv'))
    else:

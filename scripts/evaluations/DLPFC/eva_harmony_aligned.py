...
def main(samples, cluster_num, method):
    ...
    for sample in samples:
        cnt_paths.append("../../../results/tmp/DLPFC/" + sample + "_cnt_aligned.mtx")
        xy_paths.append("../../../results/tmp/DLPFC/" + sample + "_xy_aligned.csv")
    ...
    if type(clusters) == 'rpy2.robjects.vectors.ListVector':
        for sample in clusters.names:
            ...
            df.to_csv(os.path.join("../../../results/tmp/DLPFC/harmony_aligned", '_'.join([sample, method]) + '.csv'))
    else:
